...
public abstract class IGeneralParser extends IParser {
	...
	protected String parseGeneral() {
		...
		String linkUrl = app.getAppHostUrl() + File.separator + IParserRegexConstant.HTML_SAVE_PATH + File.separator + app.getAppId();
	...
	protected String getWebsiteUrl() {
		if (!StringUtil.isBlank(mobilePath)) {
			return app.getAppHostUrl() + File.separator + IParserRegexConstant.HTML_SAVE_PATH + File.separator + app.getAppId() + File.separator + mobilePath;
		}
